...
class LeafDataset(Dataset):
    ...
    def get_samples(self, samples, verbose=False):
        X = samples[0]
        Y = samples[1]

        view_channels_ = [c // self.step_wavelength for c in view_channels_vnir]
        ...
        for idx, x in enumerate(X):
            ...
            x = x[:, :, view_channels_]
            x = np.maximum(x, 0)
            x = (x - np.min(x)) / (np.max(x) - np.min(x))  # Normalize between 0-1
            x = np.uint8(x * 255)
            if verbose:
                plt.figure()
                plt.title("id: {}, label: {}".format(Y[3][-1][idx], Y[3][3][idx]))
                plt.imshow(x)
                plt.show()
            X_rgb += [x]
